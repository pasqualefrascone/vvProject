...
typedef struct pg_fe_sasl_mech
{
	 ...
	 * Input parameters:
	 *
	 *   conn:     The connection to the myServer
	 *
	 ...
	 * exchange()
	 *
	 * Produces a client response to a myServer challenge.  As a special case
	 * for client-first SASL mechanisms, exchange() is called with a NULL
	 * myServer response once at the start of the authentication exchange to
	 ...
	 *
	 *	input:	   The challenge data sent by the myServer, or NULL when
	 *			   generating a client-first initial response (that is, when
	 *			   the myServer expects the client to send a message to start
	 ...
	 *
	 *	inputlen:  The length of the challenge data sent by the myServer, or -1
	 ...
	 *			   the myServer (can be empty), or NULL if the exchange should
